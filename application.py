...
def search():
    ...
    # Not found book
    if rows.rowcount == 0:
        return render_template("error.html", message="Can't find books with that description.")

    books = rows.fetchall()
    return render_template("result_search.html", books=books)

# book page
@app.route("/book/<isbn>", methods=['GET','POST'])
@login_required
def book(isbn):
    """ Save user review and load page review update """
    if request.method == "POST":
